...
sampleInput :: [[UnverifiedProductId]]
...
actualInput :: IO [[UnverifiedProductId]]

actualInput = readFile "data/data.txt" &  fmap  (map parseInput . splitOn ",")
...
main = hspec $ do
  ...
  describe "Summing all invalid ids over a collection of id ranges" $ do
    it "Returns 1227775554 " $ do
      calcInvalidIdsSum sampleInput `shouldBe` NonNegative 1227775554

  describe "Summing all invalid ids over the input id ranges" $ do
    it "Returns 29940924880 " $ do
      calcInvalidIdsSum <$> actualInput `shouldReturn` NonNegative 29940924880
    ...
    describe "Identifying a valid id" $ do
      ...
      describe "Summing all invalid ids over a collection of id ranges" $ do
        it "Returns 4174379265" $ do
          calcInvalidIdsSum' sampleInput `shouldBe` NonNegative 4174379265

      describe "Summing all invalid ids over a large collection of id ranges" $ do
        it "Returns 48631958998" $ do
          calcInvalidIdsSum' <$> actualInput `shouldReturn` NonNegative 48631958998
